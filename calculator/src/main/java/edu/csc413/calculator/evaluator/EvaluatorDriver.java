        ...
        if (args.length == 0) {
            ...
            EvaluatorDriver.testExpressions.forEach((testExpression, expectedResult) -> {
                try {
                    ...
                    Result testResult = new Result(testExpression, expectedResult, actualResult,
                            expectedResult.equals(actualResult));
                    EvaluatorDriver.testResults.put(testExpression, testResult);
                } catch (Exception ex) {
                    Result testResult = new Result(testExpression, expectedResult, ex.toString(),
                            expectedResult.equals("exception"));
                    EvaluatorDriver.testResults.put(testExpression, testResult);
                }
            });
    ...
    static void printTestResultsForAutoDriver() {
        ...
        EvaluatorDriver.testResults.forEach((testExpression, testResultInformation) -> {
            System.out.printf("\n| %43s | %15s | %9s | %s", testExpression,
                    ...
                    testResultInformation.getPassOrFailColored(),
...
class Result {
    ...
    public Result(String testExpression, String expectedResult, String actualResult, boolean passOrFail) {
    ...
    public String getPassOrFailColored() {
        if (this.passOrFail) {
            return String.format("%s %s %s %s", GREEN_BACKGROUND, CHECKMARK, "pass", RESET);

        } else {
            return String.format("%s %s %s %s", RED_BACKGROUND, CROSS, "fail", RESET);
        }
